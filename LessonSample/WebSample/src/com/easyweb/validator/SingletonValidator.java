...
public class SingletonValidator {
	...
	private SingletonValidator() {
		...
		try {
			...
			for (String str : ClassUtils.getAllFilesInClassPath((URLClassLoader) this.getClass().getClassLoader()).stream().filter(x -> x.endsWith("-Validator.xml")).collect(Collectors.toList())) {
